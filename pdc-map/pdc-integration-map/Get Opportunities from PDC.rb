# Get All Opportunities from PDC
# Returns array of all opportunities, handling pagination automatically
...
begin
  loop do
    ...
    uri = URI.parse("#{base_url}/opportunities")
    ...
    when "200"
      ...
      batch = data['entries'] || []
      opportunities.concat(batch)
      
      # Check if we've fetched everything
      # Stop if no more entries or we've reached the total
      break if batch.empty?
      break if opportunities.length >= total_count
    ...
    when "404"
      raise "Opportunities endpoint not found at #{uri}"
    else
